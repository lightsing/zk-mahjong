...
import './App.css'
import { MahjongKey, AggregatedMahjongPubkey, genInitTileSet } from 'zk-mahjong-wasm'
import TileComponent from './components/Tile'
import { idToTile } from './utils/tile'

function App() {
  const secretKeyList = Array.from({ length: 4 }).map(_ => { return new MahjongKey() })
  const publicKeyList = secretKeyList.map(e => e.publicKey)

  const aggregatePublicKey = new AggregatedMahjongPubkey(publicKeyList)

  const { permutation, randomness, tiles } = genInitTileSet(aggregatePublicKey)
