...
task('compile-circuit', 'Compile the circuit')
    ...
    .setAction(async ({ name, degree }, hre) => {
        ...
        execSync(
            `circom circuits/${name}.circom --r1cs --sym --wasm --O2 --output build/${name}`
        )
        execSync(
            `snarkjs plonk setup build/${name}/${name}.r1cs cache/${degree}.ptau build/${name}/${name}.zkey`
        )
        execSync(
            `snarkjs zkey export verificationkey build/${name}/${name}.zkey build/${name}/verification_key.json`
        )
        execSync(
            `snarkjs zkey export solidityverifier build/${name}/${name}.zkey contracts/${name}.sol`
        )
    })
